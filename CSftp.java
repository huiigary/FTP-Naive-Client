...
public class CSftp {
	...
	//function:  if there is correct number of args , then set IP and PORT values, else exit
	...
	//function: connect to server socket and outputs response from server
	static private void connect(String IP, Integer PORT) {
		try {
			...
			while (run) {   // principal loop that keeps conversation going
				if (repeat){
					execute(out, in, stdIn, fromServer);
				}
				else {
	...
	private static void sleep(BufferedReader in) throws IOException {
		...
		while(System.currentTimeMillis()-time < 200){ //1000
			if(in.ready()){
				break;
			}
		}
	...
	//function: Prints "CSFTP" prompt and executes commands ("user , pw , quit , dir, get, features") inputted by User.
	static private void execute(PrintWriter out, BufferedReader in, BufferedReader stdIn, String fromServer) {
		try {
			System.out.print("csftp> ");

			String fromUser = stdIn.readLine();
			if (fromUser.startsWith("#") || fromUser.startsWith(" ")){  // invalid inputs "#"  && " "  fromUser.startsWith(" ")
				repeat = true;
				return;
			}
			if (fromUser != null) { 
	...
	// helper function :  prints all lines given in FEATURES from the server
	...
	// helper function: takes in PASV mode values , then connects to Socket.  then execute "LIST" and prints server response
	static private void dir(Socket s2,PrintWriter out,BufferedReader in, String fromServer) {
		...
		try {
			...
			while ((fromServer2 = in2.readLine()) != null) {
				...
				if(fromServer.contains("226")){                 // if server yields :226 Closing data connection.//TODO check this
				...
				if (fromServer.contains("150") || fromServer.contains("125" )) {                // opening binary mode data connection
					try {
						...
						initialStream.read(buffer);
							...
							if (new File(fileName).exists()){                                    //if file already exists on local computer throw error
								s3.close();
								repeat = false;
                                out.print("ABOR" + "\r\n");
                                out.flush();
                                throw new FileAlreadyExistsException("0x38E Access to local file " + remote + " denied.");//TODO maybe fix how this gets thrown
							}

							FileOutputStream outStream = new FileOutputStream(fileName);
							int length;
							while ((length = initialStream.read(buffer)) > 0) {
